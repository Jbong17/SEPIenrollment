...
def page_enroll():
    ...
    # ── Step 2: Academic ──────────────────────────────────────────────────────
        ...
        if f["level"] == "shs":
            f["strand"] = st.selectbox("Strand *", STRANDS,
                index=STRANDS.index(f["strand"]) if f.get("strand") in STRANDS else 0)
        else:
            f["strand"] = ""
    ...
    # ── Step 4: Documents ─────────────────────────────────────────────────────
        ...
        if f.get("level") == "shs":
            docs_list.append("JHS Diploma / Completion Certificate")
            docs_list.append("ESC Certificate (if applicable)")
...
def _student_status(s):
    ...
    if s.get("level") == "shs":
        docs_list += ["JHS Diploma / Completion Certificate","ESC Certificate (if applicable)"]
